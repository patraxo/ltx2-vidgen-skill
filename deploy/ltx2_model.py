...
class Model:
    ...
    def generate(
        self,
        request: Request,
        image_urls: list[str] = Body(None, description="List of image URLs (1 for I2V, 2+ for keyframe interpolation)"),
        image_data: list[str] = Body(None, description="List of base64-encoded images (alternative to image_urls)"),
        prompt: str = Body("", description="Text prompt describing the desired video content"),
        # 2026-05-23: default is now the upstream-canonical 147-word DEFAULT_NEGATIVE_PROMPT
        # (mirrored at top of this file from ltx_pipelines.utils.constants @ commit 76730e6).
        # Previously a 14-word stub which under-constrained the artifact space the CFG
        # branch is supposed to subtract.
        negative_prompt: str = Body(
            DEFAULT_NEGATIVE_PROMPT,
            description="Negative prompt for unwanted content (defaults to Lightricks-canonical 147-word list)"
        ),
        num_frames: int = Body(121, ge=9, le=257, description="Number of frames (must be 8*K+1, e.g., 9, 17, 25, ..., 121, 257)"),
        height: int = Body(1024, ge=256, le=2048, description="Video height in pixels (divisible by 64 for two-stage)"),
        width: int = Body(1536, ge=256, le=2048, description="Video width in pixels (divisible by 64 for two-stage)"),
        frame_rate: float = Body(24.0, ge=1.0, le=60.0, description="Frame rate (fps)"),
        # 2026-05-23 Tier-1A: optional w/ mode-aware default. Resolution
        # happens inside this handler before the call into `_process_video`:
        #   * default mode (Euler + gradient-estimating, ge_gamma=2.0) → 20
        #   * hq mode (res_2s second-order) → 30 (unchanged)
        # Callers passing an explicit integer still pin to that value
        # (preserves backward-compat for test_stormlight.py, szeth_prologue.py,
        # test_ltx2_v2.py — none of them rely on the default).
        num_inference_steps: int | None = Body(None, ge=1, le=100, description="Number of denoising steps (default: 20 for mode='default' w/ gradient-estimating Euler, 30 for mode='hq')"),
        cfg_guidance_scale: float = Body(3.0, ge=1.0, le=20.0, description="Classifier-free guidance scale"),
        seed: int = Body(42, description="Random seed (-1 for random)"),
        enhance_prompt: bool = Body(False, description="Auto-enhance the prompt using the model"),
        mode: str = Body(
            "default",
            description=(
                "Pipeline variant: 'default' = Euler two-stage (i2v + keyframe interp); "
                "'hq' = TI2VidTwoStagesHQ (res_2s sampler, single-image only, slower but sharper); "
                "'preview' = DistilledPipeline (~11 total denoising ops, single-image only, "
                "no negative_prompt / CFG / num_inference_steps — fastest path, lower quality)."
            ),
        ),
        enable_step_cache: bool | None = Body(
            None,
            description=(
                "First-Block Cache step skipping on stage_1 of the Euler two-stage "
                "path (i2v + KF interp). Tri-state:\n"
                "  * None (default) → ON for mode='default' (i2v + KF interp). "
                "research_v2 measured ~1.5-2x wall-clock win on the Euler stage_1 "
                "with no perceptible quality loss at threshold 0.05 / max_skip 3.\n"
                "  * True  → force ON (same as the default-mode auto-on).\n"
                "  * False → force OFF (escape hatch for quality-critical default-mode "
                "renders).\n"
                "Always ignored when mode='hq' (res_2s breaks block-residual caching; "
                "research_v2: 'every step matters') and mode='preview' (no CFG branch), "
                "and never applied to stage_2 or Retake."
            ),
        ),
        step_cache_threshold: float = Body(
            0.05,
            ge=0.0,
            le=0.5,
            description=(
                "Relative-L1 threshold for FBCache cache-hit. 0.05 conservative "
                "(diffusers default for 22B-class DiTs), 0.10 aggressive. Only "
                "consulted when enable_step_cache=True."
            ),
        ),
        guider: str = Body(
            "cfg",
            description=(
                "Stage-1 guidance variant. 'cfg' = vanilla MultiModalGuider (default, "
                "back-compat). 'apg' = Adaptive Projected Guidance (project (cond - uncond) "
                "onto cond, scale parallel by apg_eta). 'cfg_star' = CFG★ rescaling "
                "(rescale uncond by its projection onto cond). Both 'apg' and 'cfg_star' "
                "are rejected when mode='hq' (HQ pipeline bypasses the factory hook). "
                "Calibration: APG typically wants cfg_guidance_scale~1.5 (vs 3.0 for CFG); "
                "CFG★ uses the same cfg_guidance_scale as CFG."
            ),
        ),
        apg_eta: float = Body(
            1.0,
            ...
            le=2.0,
            description=(
                "APG parallel-component weight (only used when guider='apg'). 1.0 = "
                "preserve full parallel component (mildest); 0.0 = drop parallel entirely "
                "(most aggressive projection); 0.5 is a common compromise. Ignored for "
                "guider='cfg' and guider='cfg_star'."
            ),
        ),
        apg_norm_threshold: float = Body(
            0.0,
            ge=0.0,
            description=(
                "APG L2-norm clip on the (cond - uncond) delta (only used when "
                "guider='apg'). 0.0 = disabled (default). 5.0-15.0 mirrors the "
                "LegacyStatefulAPGGuider default range when norm clipping is desired."
            ),
        ),
        stg_scale: float | None = Body(
            None,
            description=(
                "W8 Tier-1C: override the multi-modal guider `stg_scale` (Spatio-Temporal "
                "Guidance) on both video and audio guider params at request time. "
                "Defaults: 1.0 for mode='default' (LTX_2_3_PARAMS), 0.0 for mode='hq' "
                "(LTX_2_3_HQ_PARAMS). Set to None to keep the mode-default."
            ),
        ),
        distilled_lora_strength_stage_1: float | None = Body(
            None,
            description=(
                "W8 Tier-1D: override stage-1 distilled-LoRA strength. ONLY applies to "
                "mode='hq' (default Euler + KF interp do not apply distilled LoRA at "
                "stage 1, so this is ignored there). Triggers a pipeline rebuild on "
                "first use per (s1, s2) tuple; up to 4 variants are cached."
            ),
        ),
        distilled_lora_strength_stage_2: float | None = Body(
            None,
            description=(
                "W8 Tier-1D: override stage-2 distilled-LoRA strength for all modes. "
                "Baked-in defaults: 1.0 (default Euler + KF), 0.5 (HQ). Triggers a "
                "pipeline rebuild on first use per (mode, s1, s2) tuple; up to 4 "
                "variants are cached."
            ),
        ),
        lora: str | None = Body(
            None,
            description=(
                "Optional community LoRA to apply (mode='default' only). "
                "'transition' = joyfox LTX-2.3 shot-transition / first↔last-frame morph LoRA; "
                "'vbvr' = LiconStudio VBVR-I2V video-reasoning consistency LoRA. Both are "
                "dev-base i2v/KF LoRAs appended to the pipeline's loras= list. Rejected with "
                "mode='hq'/'preview' and with distilled_lora_strength_stage_{1,2}. The "
                "Multiple-Subject-Reference (MSR) IC-LoRA is intentionally NOT offered — it "
                "needs a custom reference-to-pseudo-video preprocessing node, not the loras= path."
            ),
        ),
        lora_strength: float | None = Body(
            None,
            ge=0.0,
            le=2.0,
            description=(
                "Strength for the selected `lora` (only used when `lora` is set). "
                "Defaults to the repo-recommended ~0.8-1.0 per LoRA when omitted."
            ),
        ),
        skip_audio: bool = Body(
            False,
            description=(
                "2026-06-07 audio-skip lever. DEFAULT FALSE = audio is KEPT (existing "
                "audio reels are unaffected). Set TRUE for video-only / silent reels: the "
                "audio VAE decoder + vocoder (~1.3s) are NOT built and a clean video-only "
                "mp4 is written. The video pixels are byte-identical to the audio-on run "
                "(audio is a separate latent; the denoise context is untouched) — only the "
                "audio track is dropped. Recommended TRUE for the @patrawtf/Hail Films "
                "video-only reel pipeline."
            ),
        ),
    ):
        """
        Generate video from images using LTX-2.3.

        Accepts images via **image_urls** (public URLs) or **image_data** (base64 strings).
        Supply one or the other (or both — they'll be merged in order).

        - **Single image, mode='default'**: TI2VidTwoStagesPipeline (Euler two-stage)
        - **Single image, mode='hq'**: TI2VidTwoStagesHQPipeline (res_2s, Lightricks's documented HQ path)
        - **Single image, mode='preview'**: DistilledPipeline (~11 total denoising ops, no CFG; fastest path, ignores `negative_prompt` / `cfg_guidance_scale` / `num_inference_steps`)
        - **Multiple images, mode='default'**: KeyframeInterpolationPipeline (Euler two-stage)
        - mode='hq' and mode='preview' with multiple images are rejected — upstream has no KF interp variant for either yet.

        Returns MP4 video file.
        """
        # Resolve images: merge URLs and base64 inputs
        resolved_urls: list[str] = []
        base64_temp_files: list[str] = []
        ...
        if image_data:
            for b64 in image_data:
                ...
                base64_temp_files.append(path)
                resolved_urls.append(f"file://{path}")

        if not resolved_urls:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Provide at least one image via image_urls or image_data",
            )

        # Validate num_frames (must be 8*K+1)
        if (num_frames - 1) % 8 != 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"num_frames must be 8*K+1 (e.g., 9, 17, 25, ..., 121). Got {num_frames}"
            )

        # Validate resolution for two-stage (must be divisible by 64)
        if height % 64 != 0 or width % 64 != 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Height and width must be divisible by 64 for two-stage pipeline. Got {height}x{width}"
            )
        ...
        # 2026-05-23 Tier-1A: mode-aware default for num_inference_steps.
        # gradient-estimating Euler converges at ~20 steps for the default
        # mode; res_2s HQ keeps 30. mode_norm here is a lightweight
        # endpoint-side guess; `_process_video` re-validates it.
        if num_inference_steps is None:
            num_inference_steps = 30 if (mode or "default").lower() == "hq" else 20

        # FBCache default ON for default-mode (2026-06-07). The all-configs sweep
        # FRAME-VERIFIED it safe at high motion (tau=0.05, 70% step-cache hits on a
        # violent sword-swing, mid-swing frame fully coherent, ~17% faster 21→17s, no
        # visible quality loss — near-lossless, not bit-identical). Confirms the
        # VIDEO_BENCHMARK finding. An explicit enable_step_cache=false still forces
        # the strict bit-identical path. NEVER applied to mode='hq' (res_2s — every
        # step matters) or mode='preview'; _process_video re-gates those regardless.
        _mode_norm = (mode or "default").lower()
        if enable_step_cache is None:
            enable_step_cache = (_mode_norm == "default")

        print(f"\n{'='*60}")
        print(f"🎬 LTX-2.3 VIDEO GENERATION REQUEST")
        print(f"{'='*60}")
        print(f"   Images: {len(resolved_urls)} ({len(image_urls or [])} url + {len(image_data or [])} base64)")
        print(f"   Prompt: {prompt[:100]}...")
        print(f"   Resolution: {width}x{height}")
        print(f"   Frames: {num_frames} @ {frame_rate} fps")

        try:
            # Routes via `_process_video_safe` which adds OOM recovery,
            # post-request GC, and peak-memory logging. The happy path costs
            # ~50-200ms extra; the OOM path triggers a one-shot retry rather
            # than a 500 to the client.
            video_bytes = self._process_video_safe(
                ...
                enhance_prompt=enhance_prompt,
                mode=mode,
                enable_step_cache=enable_step_cache,
            ...
            return Response(
                ...
                headers={
                    "Content-Disposition": "attachment; filename=ltx2_output.mp4",
                }
            )
    ...
    def generate_control(
        self,
        request: Request,
        control_video_url: str = Body(..., description="URL (or file://) to the positionally-aligned control/reference video (e.g. Canny/Depth/Pose render for union, motion-track render for motion_track)"),
        prompt: str = Body("", description="Text prompt describing the desired video content"),
        mode: str = Body(
            "union",
            description=(
                "IC-LoRA control type: 'union' = Union-Control (Canny + Depth + Pose; "
                "the 22B camera/motion-control path) or 'motion_track' = "
                "Motion-Track-Control (trajectory/point motion). Both run upstream "
                "ICLoraPipeline on the distilled base."
            ),
        ),
        image_urls: list[str] = Body(None, description="Optional: list of image URLs; image_urls[0] is used as an initial-frame (i2v) conditioning image"),
        image_data: list[str] = Body(None, description="Optional: base64-encoded init image(s) (alternative to image_urls)"),
        num_frames: int = Body(121, ge=9, le=257, description="Number of frames (must be 8*K+1)"),
        height: int = Body(1024, ge=256, le=2048, description="Video height (divisible by 64)"),
        width: int = Body(1536, ge=256, le=2048, description="Video width (divisible by 64)"),
        frame_rate: float = Body(24.0, ge=1.0, le=60.0, description="Frame rate (fps)"),
        seed: int = Body(42, description="Random seed (-1 for random)"),
        control_strength: float = Body(1.0, ge=0.0, le=1.0, description="Strength of the control/reference video conditioning (the upstream video_conditioning strength)"),
        conditioning_attention_strength: float = Body(1.0, ge=0.0, le=1.0, description="IC-LoRA conditioning attention strength (0.0 = ignore control, 1.0 = full influence)"),
        enhance_prompt: bool = Body(False, description="Auto-enhance the prompt"),
    ):
        """
        Generate video with a 22B IC-LoRA control signal (Union or Motion-Track).

        Supply a positionally-aligned **control_video_url** (the reference signal —
        e.g. a Canny/Depth/Pose render for `union`, or a motion-track render for
        `motion_track`). Optionally supply an init image via **image_urls** /
        **image_data** for i2v-style first-frame conditioning.

        Returns MP4 video file.
        """

        mode_norm = (mode or "union").lower()
        if mode_norm not in ("union", "motion_track"):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Unknown control mode: {mode!r}. Use 'union' or 'motion_track'.",
            )

        if not control_video_url:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="control_video_url is required for IC-LoRA control generation.",
            )

        # Resolve optional init images (URLs + base64) into a single URL list.
        resolved_image_urls: list[str] = []
        if image_urls:
            resolved_image_urls.extend(image_urls)
        if image_data:
            for b64 in image_data:
                path = self._save_base64_image(b64)
                resolved_image_urls.append(f"file://{path}")

        if (num_frames - 1) % 8 != 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"num_frames must be 8*K+1. Got {num_frames}",
            )
        if height % 64 != 0 or width % 64 != 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Height and width must be divisible by 64. Got {width}x{height}",
            )
        if seed == -1:
            seed = int.from_bytes(os.urandom(4), "big") % (2**31)

        print(f"\n{'='*60}")
        print(f"🎬 LTX-2.3 IC-LoRA CONTROL REQUEST (mode={mode_norm})")
        print(f"{'='*60}")
        print(f"   Control video: {control_video_url[:80]}")
        print(f"   Init images: {len(resolved_image_urls)}")
        print(f"   Resolution: {width}x{height}  Frames: {num_frames} @ {frame_rate} fps")

        try:
            video_bytes = self._process_ic_lora_safe(
                control=mode_norm,
                ...
                frame_rate=frame_rate,
                seed=seed,
                image_urls=resolved_image_urls or None,
            ...
            return Response(
                content=video_bytes,
                media_type="video/mp4",
                headers={"Content-Disposition": "attachment; filename=ltx2_control.mp4"},
            )
        except HTTPException:
            raise
        except Exception as e:
            logger.error(f"Error during IC-LoRA control generation: {str(e)}")
            import traceback
            logger.error(traceback.format_exc())
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"IC-LoRA control generation failed: {str(e)}",
            )
    ...
    def retake(
        self,
        request: Request,
        video_url: str = Body(None, description="URL to source video"),
        video_data: str = Body(None, description="Base64-encoded source video (alternative to video_url)"),
        prompt: str = Body(..., description="Text prompt describing the regenerated section"),
        start_time: float = Body(..., ge=0.0, description="Start of region to regenerate (seconds)"),
        end_time: float = Body(..., description="End of region to regenerate (seconds)"),
        # 2026-05-23: align retake fallback with upstream-canonical DEFAULT_NEGATIVE_PROMPT
        # (was a 6-word stub).
        negative_prompt: str = Body(
            DEFAULT_NEGATIVE_PROMPT,
            description="Negative prompt for unwanted content (defaults to Lightricks-canonical 147-word list)"
        ),
        num_inference_steps: int = Body(30, ge=1, le=100, description="Number of denoising steps"),
        cfg_guidance_scale: float = Body(3.0, ge=1.0, le=20.0, description="Classifier-free guidance scale"),
        seed: int = Body(42, description="Random seed (-1 for random)"),
        regenerate_video: bool = Body(True, description="Regenerate video in the time window"),
        regenerate_audio: bool = Body(True, description="Regenerate audio in the time window"),
        enhance_prompt: bool = Body(False, description="Auto-enhance the prompt"),
    ):
        """
        Video-to-video retake: regenerate a time region of an existing video.

        Keeps everything outside [start_time, end_time] unchanged and regenerates
        the content inside from the text prompt. Source video must have frame count
        satisfying 8k+1 and resolution divisible by 32.

        Accepts source video via **video_url** or **video_data** (base64).
        Returns MP4 video file.
        """
        ...
        temp_video = None
        if video_data:
            temp_video = self._save_base64_video(video_data)
            resolved_path = temp_video
        elif video_url:
            resolved_path = self._download_video(video_url)
            if not resolved_path.startswith("/"):
                temp_video = resolved_path
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Provide a source video via video_url or video_data",
            )

        if start_time >= end_time:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"start_time ({start_time}) must be less than end_time ({end_time})",
            )
        ...
        print(f"\n{'='*60}")
        print(f"🎬 LTX-2.3 VIDEO RETAKE REQUEST")
        print(f"{'='*60}")
        print(f"   Prompt: {prompt[:100]}...")
        print(f"   Window: {start_time:.1f}s - {end_time:.1f}s")

        try:
            # Routes via `_process_retake_safe` for OOM-aware retry + post-call GC.
            video_bytes = self._process_retake_safe(
            ...
            return Response(
                ...
                media_type="video/mp4",
                headers={
                    "Content-Disposition": "attachment; filename=ltx2_retake.mp4",
                }
            )
